    ...
    <>
      ...
      {rocks.length === 0 ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl py-12 text-center mb-8">
          <p className="text-zinc-500 text-sm">No rocks defined yet for {quarterLabel}.</p>
        </div>
      ) : (
        <div className="space-y-2 mb-8">
          {rocks.map(rock => {
            ...
            return (
              <div key={rock.id} className="bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white">{rock.title}</p>
                  {rock.description && <p className="text-xs text-zinc-400 mt-0.5">{rock.description}</p>}
                  <p className="text-xs text-zinc-500 mt-1">Owner: {rock.owner}</p>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0 mt-0.5">
                  <button onClick={() => openEdit(rock)} className="text-xs text-zinc-500 hover:text-zinc-200 transition-colors">Edit</button>
      ...
      {deleting && (
        <Modal title="Delete Rock" onClose={() => setDeleting(null)}>
          <p className="text-zinc-300 text-sm mb-4">Are you sure you want to delete this rock? This cannot be undone.</p>
          <p className="text-zinc-400 text-sm italic mb-5">&ldquo;{deleting.title}&rdquo;</p>
