...
export default function GoalsManager({ goals: initial, workspaceId }: Props) {
  ...
  const STATUS_COLORS: Record<string, string> = {
    active: 'bg-blue-500/20 text-[#1B84FF] border-blue-500/30',
    achieved: 'bg-green-500/20 text-green-400 border-green-500/30',
    paused: 'bg-yellow-500/20 text-[#f6a600] border-yellow-500/30',
    cancelled: 'bg-[#f1f1f4] text-[#78829d] border-[#e8e8e8]',
  }

  return (
    <>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-[#78829d] uppercase tracking-wider">Goals</h2>
        <button
          onClick={openAdd}
      ...
      {goals.length === 0 ? (
        <div className="bg-white border border-[#e8e8e8] rounded-xl py-12 text-center mb-8">
          <p className="text-[#78829d] text-sm">No goals defined yet.</p>
        </div>
      ) : (
        <div className="space-y-2 mb-8">
          {goals.map(goal => (
            <div key={goal.id} className="bg-white border border-[#e8e8e8] rounded-lg px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-sm text-[#4b5675]">{goal.goalText}</p>
              ...
              <span className="text-xs text-[#78829d] shrink-0">{goal.timeframe}</span>
              <span className={`text-xs font-medium px-1.5 py-0.5 rounded border shrink-0 ${STATUS_COLORS[goal.status] ?? 'bg-[#f1f1f4] text-[#78829d] border-[#e8e8e8]'}`}>
                {goal.status.charAt(0).toUpperCase() + goal.status.slice(1)}
              </span>
              <button onClick={() => openEdit(goal)} className="text-xs text-[#78829d] hover:text-[#4b5675] transition-colors shrink-0">Edit</button>
      ...
      {(showAdd || editing) && (
        <Modal title={editing ? 'Edit Goal' : 'Add Goal'} onClose={() => { setShowAdd(false); setEditing(null) }}>
          <div className="space-y-3">
            <div>
              <label className="block text-xs text-[#78829d] mb-1">Goal *</label>
              <textarea
                value={form.goalText}
            ...
            <div>
              <label className="block text-xs text-[#78829d] mb-1">Timeframe *</label>
              <input
            ...
            <div>
              <label className="block text-xs text-[#78829d] mb-1">Revenue Target ($)</label>
              <input
                type="number"
      ...
      {deleting && (
        <Modal title="Delete Goal" onClose={() => setDeleting(null)}>
          <p className="text-[#4b5675] text-sm mb-4">Are you sure you want to delete this goal? This cannot be undone.</p>
          <p className="text-[#78829d] text-sm italic mb-5">&ldquo;{deleting.goalText}&rdquo;</p>
