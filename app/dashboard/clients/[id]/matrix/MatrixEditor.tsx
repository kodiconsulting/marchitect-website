                  ...
                  <tbody>
                    {byCategory[cat].map(row => {
                      ...
                      return (
                        <tr
                          key={row.functionId}
                          className={`border-t border-zinc-800 transition-colors ${isSaving ? 'opacity-60' : 'hover:bg-zinc-800/50'}`}
                        >
                          ...
                          <td className="px-4 py-3">
                            {editing ? (
                              <input
                                ...
                                className="w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-zinc-200 placeholder-zinc-600 text-sm outline-none focus:border-blue-500"
                              />
                            ) : (
                              <div className="flex items-center gap-2">
                                <span className="text-zinc-400">{row.assignedOwner ?? 'Unassigned'}</span>
                                {!row.assignedOwner && (
                          ...
                          <td className="px-4 py-3">
                            {editing ? (
                              <select
                                ...
                                className="bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-zinc-200 text-sm outline-none focus:border-blue-500 cursor-pointer"
                              >
                                <option value="">—</option>
                              ...
                              <span className="text-zinc-500">{row.internalExternal ?? '—'}</span>
                            )}
                          </td>
                            ...
                            {editing ? (
                              <div className="flex items-center gap-3">
                                {(['gwcGet', 'gwcWant', 'gwcCapacity'] as const).map((key, i) => (
                                  <label key={key} className="flex items-center gap-1 cursor-pointer text-zinc-400 text-xs select-none">
                                    <input
                                      type="checkbox"
                              ...
                              <span className="text-zinc-500">{gwcLabel(row)}</span>
                            )}
                          </td>
